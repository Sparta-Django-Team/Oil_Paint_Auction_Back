...
subject='오코완 휴면회원 계정 처리 안내'
content='고객님 안녕하세요. 오코완입니다. 저희는 소중한 고객님의 개인정보 보호를 위해 관계 법렵에 따라 고객님의 온라인 계정을 별도로 분리 보관할 예정입니다. 휴면계정을 해제하기 위해서는 별도의 인증 동의 절차가 진행될 수 있으므로 편리한 계정 사용이 필요하시다면 지금 바로 오코완을 방문해주세요.'

if user:
    for i in user_email:
        mail = EmailMessage(subject, content, to=[i["email"]])
        mail.send()
