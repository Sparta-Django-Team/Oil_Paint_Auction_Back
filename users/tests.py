...
class UserProfileAPIViewTestCase(APITestCase):
    @classmethod
    def setUpTestData(self):
        self.user_data = {"email": "test1@test.com", "password":"Test1234!"}
        User.objects.create_user("test1@test.com","test1","Test1234!")
        User.objects.create_user("test2@test.com","test2","Test1234!")
        self.user = authenticate(email="test1@test.com", password="Test1234!")
    ...
    # 프로필 접근 실패 (비회원 & not found)
    def test_user_profile_get_not_found_fail(self):
        response = self.client.get(
            path=reverse("user_profile"),
        )
    ...
    def test_user_profile_put_success(self):
        ...
        response = self.client.put(
            path=f'{reverse("user_profile")}?token={self.token}',
    ...
    def test_user_profile_put_nickname_validation_fail(self):
        response = self.client.put(
            path=f'{reverse("user_profile")}?token={self.token}',
            HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
            data={"nickname": "test111!"},
        )
        self.assertEqual(response.status_code, 400)

    # 회원정보 수정 실패(닉네임 중복)
    def test_user_profile_put_nickname_unique_fail(self):
        response = self.client.put(
            path=f'{reverse("user_profile")}?token={self.token}',
            HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
            data={"nickname": "test2"},
    ...
    # 회원정보 수정 실패(닉네임 빈칸)
    def test_user_profile_put_nickname_blank_fail(self):
        response = self.client.put(
            path=f'{reverse("user_profile")}?token={self.token}',
            HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
            data={"nickname": ""},
    ...
    # 회원정보 수정 실패(토큰 에러)
    def test_user_profile_put_token_invalid_fail(self):
        response = self.client.put(
            path=f'{reverse("user_profile")}?token={self.token}1',
            HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
            data={"nickname": "test2"},
        )
        self.assertEqual(response.status_code, 401)